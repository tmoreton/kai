...
// Templates feature removed - spawnFromTemplate no longer available
// import { spawnFromTemplate } from "./templates.js";
import type { AgentEvent } from "./types.js";
import crypto from "crypto";
import { getDb, saveAgent } from "../agents-core/db.js";
...
async function spawnAndWait(subGoal: SubGoal, parentGoalId: string): Promise<unknown> {
  // Create agent directly (templates feature removed)
  const agentId = crypto.randomUUID();
  const agentName = `Goal-${parentGoalId.slice(0, 8)}-${subGoal.id.slice(0, 8)}`;
  saveAgent({
    id: agentId,
    name: agentName,
  ...
  console.log(`[Orchestrator] Spawned agent ${agentId} for ${subGoal.id}`);
